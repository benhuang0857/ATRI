...
class AwardExcelController extends Controller
{
    public function index(Request $req)
    {
        ...
        $companies = Award::with('CompanyBasicInfo')
                                ->where('award_time', '>=',  $setYear.'-01-01')
                                ->where('award_time', '<=',  $setYear.'-12-31')
                                ->get();
        ...
        return $content
            ->title('申請/取得獎項')
            ->description('統計各所政獎項資源')
            ->view('adv-excel.award-export', [
                'title' => '申請/取得獎項',
                'dataset' => $datasets,
                'jscode'  => $jscode
            ]);;
